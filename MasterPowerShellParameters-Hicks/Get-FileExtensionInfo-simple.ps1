...
function Get-FileExtensionInfo {
    ...
    process {
        ...
        if ($Hidden) {
            ...
            Get-ChildItem -Path $Path -File -Hidden -Recurse:$Recurse | ForEach-Object { $fileList.Add($_) }
        }

        #make a second pass to get all files
        Get-ChildItem -Path $Path -File -Recurse:$Recurse | ForEach-Object { $fileList.Add($_) }
